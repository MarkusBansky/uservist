...
public interface UserDao extends JpaRepository<User, Long> {
    @Query(nativeQuery = true, value =
            "select * from users.users u left join users.user_service_connections sc on u.user_id = sc.user_id " +
                    "where sc is not null and sc.service_id = :serviceId order by u.user_id --#pageable\\n")
